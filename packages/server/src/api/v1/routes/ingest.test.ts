...
async function seedMembership(
  ...
  await db.insert(memberships).values({ user_id: userId, semester_id: semesterId, role, granted_by: userId });
}
...
function makeMultipartRequest(
  ...
  for (const f of files) {
    formData.append(f.field, new Blob([f.bytes.buffer as ArrayBuffer], { type: 'application/zip' }), f.name);
  }
  ...
  it('returns 202 with job_id when staging multiple files', async () => {
    await withTestDb(async (db) => {
      await withTestMinio(async ({ client, bucketName }) => {
        _testDb = db;
        try {
          _setConfigForTest(parseEnv(makeTestEnv(client.bucketUrl.replace(`/${bucketName}`, ''), bucketName)));
      ...
      await withTestMinio(async ({ client, bucketName }) => {
        ...
        try {
          ...
          const cancelRes = await app.fetch(
            new Request(
              `http://localhost/semesters/${semester.id}/ingest/jobs/${job_id}/cancel`,
              {
                method: 'POST',
                headers: { Cookie: `__Host-prov_sess=${sessionId}` },
              },
