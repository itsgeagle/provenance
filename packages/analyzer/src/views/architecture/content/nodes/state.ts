...
export const nodes: Record<string, ArchNode> = {
  dormant: {
    title: 'Dormant',
    body: 'The extension is loaded at all only because VS Code matched one of its two activation events — a workspace containing a file named .provenance-manifest or provenance-manifest, at any depth. In every other folder the extension host never wakes it, so dormant is less a running state than the absence of one.\n\nDormant is also where a workspace lands when a manifest is present but does not verify, and the recorder keeps exactly one foothold there: a stub registration for the Prepare Submission Bundle command that does nothing but explain itself when invoked. Without it, a student who runs the command in the wrong folder gets VS Code’s opaque “command not found”. The stub writes no files and records nothing, so the “does nothing” guarantee still holds.',
    links: [
