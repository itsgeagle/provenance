...
describe('alert config', () => {
  ...
  it('parses a webhook url + recipients array', () => {
    const env = parseEnv({ ...VALID_BASE, ALERT_WEBHOOK_URL: 'https://discord.test/hook',
...
export interface RenderedEvent {
  severity: Severity; kind: string; title: string; text: string; discordContent: string;
}
...
const LEVEL: Record<NotifyEvent['severity'], 'info' | 'warn' | 'error'> =
  { info: 'info', warn: 'warn', critical: 'error' };
...
export function createNotifier(deps: { sinks: Sink[]; logger: Logger }): Notifier {
  ...
  return {
    ...
    async flush(): Promise<void> { await Promise.allSettled([...inFlight]); },
  };
}
...
export function _resetNotifierForTest(): void { _notifier = null; }
```
...
describe('Throttler', () => {
  ...
  it('evicts oldest beyond maxKeys', () => {
    let t = 0; const th = new Throttler({ windowMs: 1000, now: () => t, maxKeys: 2 });
    th.admit('a'); t++; th.admit('b'); t++; th.admit('c'); // 'a' evicted
...
it('POSTs a discord-shaped body', async () => {
  ...
  const fetchImpl = (async (url: string, init: RequestInit) => {
    calls.push({ url, init }); return new Response('', { status: 204 });
  }) as unknown as typeof fetch;
  const sink = createWebhookSink({ url: 'https://d/hook', minSeverity: 'warn', timeoutMs: 1000, fetchImpl, logger });
  await sink.send({ severity: 'warn', kind: 'k', title: 'Disk 85%', text: '...', discordContent: 'X' });
...
it('rejects on non-2xx', async () => {
  ...
  const sink = createWebhookSink({ url: 'https://d/hook', minSeverity: 'warn', timeoutMs: 1000, fetchImpl, logger });
  await expect(sink.send({ severity:'warn',kind:'k',title:'t',text:'t',discordContent:'X' })).rejects.toThrow();
});
```
